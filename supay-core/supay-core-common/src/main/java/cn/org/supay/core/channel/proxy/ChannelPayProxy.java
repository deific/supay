...
public abstract class ChannelPayProxy extends SupayFilterChain  {
     ...
     * @param method
     */
    public void beforeInvoke(SupayContext<? extends Request, ? extends Response> ctx, Invocation parentInvoke, Method method) {
        if (parentInvoke == null) {
            // 根从0开始
            ctx.setCurrentInvoke(new Invocation(ctx.getChannelConfig().getChannelType(),
                    0, this.targetService.getClass().getSimpleName(), method.getName()));
        } else {
            // 记录本层调用信息
            Invocation currentInvoke = new Invocation(ctx.getChannelConfig().getChannelType(),
     ...
     * @param parentInvoke
     */
    public void afterInvoke(SupayContext<? extends Request, ? extends Response> ctx, Invocation parentInvoke) {
     ...
     * @param parentInvoke
     */
    public void finishInvoke(SupayContext<? extends Request, ? extends Response> ctx, Invocation parentInvoke) {
        Invocation currentInvoke = ctx.getCurrentInvoke();
        // 开启统计且在最上层统计
    ...
    protected Object invoke(Method method, Object[] args) {
        ...
        // 上层的调用作为本层的父
        Invocation parentInvoke = ctx.getCurrentInvoke();
        try {
